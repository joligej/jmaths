...
Q detail::opr_add(const Q & lhs, const Q & rhs) {
    ...
    if (lhs.is_positive()) {
        if (rhs.is_positive()) {
            ...
            first_product.opr_add_assign_(
                detail::opr_mult(lhs.denom_, rhs.num_));
            return Q(std::move(first_product),
                     detail::opr_mult(lhs.denom_, rhs.denom_),
                     positive);
        } else {
            N first_product = detail::opr_mult(lhs.num_, rhs.denom_);
            ...
            if (difference == 0) {
                ...
                return Q(std::move(second_product),
                         detail::opr_mult(lhs.denom_, rhs.denom_),
                         negative);
            }
        }
...
std::strong_ordering detail::opr_comp(const Q & lhs, const Q & rhs) {
    ...
    if (lhs.is_positive()) {
        if (rhs.is_positive()) {
            return detail::opr_comp(lhs.num_ * rhs.denom_,
                                    rhs.num_ * lhs.denom_);
        } else {
        ...
        if (rhs.is_positive()) {
            ...
            return detail::opr_comp(rhs.num_ * lhs.denom_,
                                    lhs.num_ * rhs.denom_);
        }
    }
}
